...
public class Persistence {
    ...
    public boolean saveSession(boolean saveAs) {
		...
		try {
            ...
            Path fullName = Paths.get(board.fileName);
    ...
    public void exportSVG() {
        try {
            ...
            // 3. Define the Physical Page Size (A4 Portrait at 72 DPI = 595x842)
            // For Landscape, use: new Dimension(842, 595)
            Dimension a4Page = new Dimension(595, 842);
            svgGenerator.setSVGCanvasSize(a4Page);

            // 4. Paint the board content into the generator
            board.paint(svgGenerator);

            // 5. Access the Root Element to add the Scaling (viewBox)
            Element svgRoot = svgGenerator.getRoot();
            
            // Get your board's actual drawing dimensions
            double boardWidth = board.getWidth();
            double boardHeight = board.getHeight();

            // The viewBox maps your board coordinates to the A4 page size
            // Syntax: "min-x min-y width height"
            String viewBoxValue = String.format("0 0 %f %f", boardWidth, boardHeight);
            svgRoot.setAttributeNS(null, "viewBox", viewBoxValue);
            
            // This ensures the drawing centers itself and maintains its proportions
	...
	public void load_1_2(RandomAccessFile file) throws IOException {
        ...
        // Checking types
        for (int i=0; i<board.eTypes.size();i++) {
            board.eTypes.elementAt(i).setId(i);
        }
